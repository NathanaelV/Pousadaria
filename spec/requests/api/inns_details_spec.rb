...
describe 'Inn API' do
  context 'GET /api/v1/inns' do
    it 'succesffuly' do
      ...
      expect(json_response['address']['address']).to eq 'Rua New York'
      expect(json_response['address']['number']).to eq '400'
      expect(json_response['address']['city']).to eq 'São Paulo'
      expect(json_response['address']['state']).to eq 'SP'
      expect(json_response['address']['postal_code']).to eq '09067-080'
